...
parser.add_argument('--output_dir_name', type=str, default='LearningCureves')
...
for Record in LossRecords_List:
    ...
    LossValues = []
    for Line in Lines[1:]:
        loss.append(float(str(Line).split('\n')[-2]))
    ...
    if ModelName == 'VGG19':
        ...
        fig.savefig(
        	os.path.join(
        		output_dir_path,
        		str(ModelName) + '_' + str(Record.split('/')[-1].split('_')[13]) + '_' + str(Record.split('/')[-1].split('_')[15]) + '_' + str(Record.split('/')[-1].split('_')[6]) + ".jpg"
        		),
        		dpi=300
        	)
        plt.clf()
        ...
        fig.savefig(
        	os.path.join(
        		output_dir_path, 
        		str(ModelName) + '_' + str(Record.split('/')[-1].split('_')[14]) + '_' + str(Record.split('/')[-1].split('_')[16]) + '_' + str(Record.split('/')[-1].split('_')[7]) + ".jpg"
        		), 
        		dpi=300
        	)
        plt.clf()
