...
parser.add_argument('--TargetLayer', type=str, default='', help='')
    ...
    def get_cam_weights(self,
        ...
        with torch.no_grad():
            ...
            for target, tensor in zip(targets, input_tensors):
                for idx_ in tqdm.tqdm(range(0, tensor.size(0), BATCH_SIZE)):
                    batch = tensor[idx_: idx_ + BATCH_SIZE, :]
...
_target_layer = [args.TargetLayer]
                        ...
                        for idx in model_names:
                            if idx == 'grad':
                                ...
                                for idy in range(data[0].shape[0]):
                                    target_layers = _target_layer  # e.g.,[model.features[36]]
                                    CAM_model = GradCAM(
                                        model_=model,
